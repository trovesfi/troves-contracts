...
pub trait IAumOracle<TContractState> {
    ...
    fn assert_vesu_v2_health_factor(ref self: TContractState, pool_id: ContractAddress, collateral: ContractAddress, debt: ContractAddress, user: ContractAddress, min_hf: u32, max_hf: u32);
}
...
#[starknet::interface]
pub trait IVesuPoolV2<TContractState> {
    fn pair_config(ref self: TContractState, collateral: ContractAddress, debt: ContractAddress) -> PairConfig;
...
pub mod AumOracle {
    ...
    use super::{IVesuPoolV2DispatcherTrait, IVesuPoolV2Dispatcher};
    ...
    pub impl AssetReporterImpl of super::IAumOracle<ContractState> {
        ...
        fn assert_vesu_v2_health_factor(ref self: ContractState, pool_id: ContractAddress, collateral: ContractAddress, debt: ContractAddress, user: ContractAddress, min_hf: u32, max_hf: u32) {
            let poolDisp = IVesuPoolV2Dispatcher {
                contract_address: pool_id
            };
            let pair_config = poolDisp.pair_config(collateral, debt);
            let max_ltv = pair_config.max_ltv;
            let max_ltv_u256: u256 = max_ltv.into();
            let (_, col_value, debt_value) = poolDisp
                .check_collateralization(collateral, debt, user);
            if (col_value == 0) {
                return; // (10HF) for assert healthy when 100% withdraw happens
            }
            ...
            if (health_factor_u32 < min_hf) {
                panic!("Health factor too low: {:?}", health_factor_u32);
            }
            if (health_factor_u32 > max_hf) {
                panic!("Health factor too high: {:?}", health_factor_u32);
            }
        }
    }
